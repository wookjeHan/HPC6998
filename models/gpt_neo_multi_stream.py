...
from huggingface.transformers.models.gpt_neo.configuration_gpt_neo import GPTNeoConfig
import torch
...
class LLM_MultiStream(nn.Module):
    def __init__(self, model_name, model_kwargs={}, tok_kwargs={}, device='cuda'):
        super(LLM_MultiStream, self).__init__()
        self.device = device
        # Load Model
        config = GPTNeoConfig(attn_implementation="eager")
        self.transformers = gpt_neo.GPTNeoForCausalLM(config)
